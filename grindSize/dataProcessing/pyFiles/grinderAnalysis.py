...
def main(N,preOrPost,smallestIncrement,coffeeCellSize,whichInformation):
    ...
    weightedAvgDiam=[]; weightStdDiamUpper=[]; weightStdDiamLower=[]
    iterations = ['a','b','c']
    for i in iterations:
    ...
    #        valuesArray = np.array(pd.read_csv("{}AdjustmentData/setting%d_stats.csv".format(preOrPost) % a ))
            # eventually the format will be vario{}{}{}_stats.csv
            valuesArray = np.array(pd.read_csv("../dataFiles/setting6p{}_stats.csv".format(i)))
    ...
    #        dataArray = np.array(pd.read_csv("{}AdjustmentData/setting{}.csv".format(preOrPost,a)))
            dataArray = np.array(pd.read_csv("../dataFiles/setting6p{}.csv".format(i)))
            ...
            print("{}".format(i))
            print("single avg diam: {}".format(avgDiam[-1]))
            print("single epos: {}".format(epos))
            print("single eneg: {}".format(eneg))
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

        weightedAvgDiam.append(avgDiam[-1])
        weightStdDiamUpper.append(epos)
        weightStdDiamLower.append(eneg)

    totalWeightAvg= np.sum(weightedAvgDiam)/3
    totalVarianceUpper=[]; totalVarianceLower=[]
    # calculate the variance 
    for i in 0,1,2:
        totalVarianceUpper.append((weightStdDiamUpper[i]-totalWeightAvg)**2)
        totalVarianceLower.append((weightStdDiamLower[i]-totalWeightAvg)**2)
    print("{}".format(totalVarianceUpper))
    totalStdDevUpper = np.sqrt(np.sum(totalVarianceUpper)/3)
    totalStdDevLower = np.sqrt(np.sum(totalVarianceLower)/3)
    print("total weighted avg: {}".format(totalWeightAvg))
    print("total std dev upper: {}".format(totalStdDevUpper))
    print("total std dev lower: {}".format(totalStdDevLower))
    #totalWeightedStdUpper = np.sqrt( np.sum( (
